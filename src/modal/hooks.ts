...
const isDirectionIncluded = (swipeDirection: OrNull<Direction>, direction: Direction) => {
  return Array.isArray(swipeDirection)
    ? swipeDirection.includes(direction)
    : swipeDirection === direction;
};

const getAccDistancePerDirection = (gestureState: PanResponderGestureState, currentSwipingDirection: OrNull<Direction>) => {
...
export const usePanResponder = (props: {
  scrollTo: any
  scrollOffset: any
  onSwipeStart: any
  backdropOpacity: any
  ...
  const { propagateSwipe, swipeDirection, swipeThreshold, onSwipeMove, onSwipeStart, scrollTo,
    panResponderThreshold, backdropOpacity, scrollOffsetMax, onSwipeCancel, onSwipeComplete, scrollHorizontal,
    scrollOffset,
    deviceHeight,
    deviceWidth
  } = props
  ...
  const isSwipeDirectionAllowed = useCallback(({ dy, dx }: PanResponderGestureState) => {
    return Boolean(
      currentSwipingDirection && isDirectionIncluded(swipeDirection, currentSwipingDirection) && {
        down: dy > 0,
        up: dy < 0,
        left: dx < 0,
        right: dx > 0,
  ...
  const calcDistanceRate = useCallback((gestureState: PanResponderGestureState) => {
    switch (currentSwipingDirection) {
      case 'down':
        return (
  ...
  const animEvt = useRef<null | ((...args: any[]) => void)>(null);

  const onMoveShouldSetPanResponder = useCallback<Required<PanResponderCallbacks>['onMoveShouldSetPanResponder']>((evt, gestureState) => {
  ...
  const onStartShouldSetPanResponder = useCallback<Required<PanResponderCallbacks>['onStartShouldSetPanResponder']>((e: any, gestureState) => {
  ...
  const onPanResponderMove = useCallback<Required<PanResponderCallbacks>['onPanResponderMove']>((evt, gestureState) => {
    ...
    if (isSwipeDirectionAllowed(gestureState)) {
      ...
      const newOpacityFactor =
        1 - calcDistanceRate(gestureState);
  ...
  const onPanResponderRelease = useCallback<Required<PanResponderCallbacks>['onPanResponderRelease']>((evt, gestureState) => {
  ...
  const getAnimationForSwipe = useCallback((): CustomAnimationType | undefined => {
    if (swipeClosing) {
